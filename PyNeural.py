...
def info_neural_network(hidden_layers,factivation,neurons_by_layer,theta):
	connections = 0
	for layer in xrange(hidden_layers+1):
		for neuron in xrange(neurons_by_layer[layer]): connections += len(theta[layer][neuron])-1
	print """"\n			  ************************
			  Neural network structure
			  ************************ """
	print "Hidden layers: ",hidden_layers
	print "Global connections: ",connections
	print 
	for layer in xrange(hidden_layers+1):
		print "*****\nLayer ",layer,"\n*****\n"
		print "Neurons in layer -> ",neurons_by_layer[layer],"\n"
		for neuron in xrange(neurons_by_layer[layer]):
			print "Neuron ",neuron," -> factivation=",factivation[layer].__name__," connections= ",len(theta[layer][neuron])-1
		print 
...
if __name__ == "__main__":
	...
	logging.info("Parameters learned! ")
	info_neural_network(hidden_layers,factivation,neurons_by_layer,theta)
	###################################################################
	...
	logging.info("Regression result: "+str(regression(x,theta,factivation)))
	########################
